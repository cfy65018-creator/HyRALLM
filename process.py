...
if __name__ == '__main__':
    ...
    # Process CCSD
    print('*'*20)
    print('start to process CCSD')
    print('*'*20)
    phase = ['train', 'valid', 'test']
    database = set()
    for n in phase:
        code_path = 'dataset/CCSD/{}/source.code'.format(n)
        nl_path = 'dataset/CCSD/{}/source.summary'.format(n)
        write_path = 'dataset/CCSD/{}.jsonl'.format(n)
        count = 0
        with open(code_path, 'r') as codes, open(nl_path, 'r') as nls, open(write_path, 'w') as file:
            for code, nl in zip(codes, nls):
                if code.strip() in database:
                ...
                file.write('\n')
                count += 1
            print('length of CCSD-{} : {}'.format(n, count))
